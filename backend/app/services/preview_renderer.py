...
def _generate_dummy_env(frontend_dir: str) -> None:
    """Populate a .env so the dev server can start without real credentials.

    Strategy (in order):
      1. Copy values from .env.example / .env.sample / .env.local.example if
         present — these often contain valid-format dummy keys that pass
         library validation (e.g. Mapbox public key format).
      2. Scan code for process.env / import.meta.env references and fill any
         remaining vars with format-aware placeholders.
    """
    import re as _re

    env_path = os.path.join(frontend_dir, ".env")

    # ── Step 1: seed from .env.example if available ────────────────────
    example_names = (
        ".env.example", ".env.sample", ".env.local.example",
        ".env.template", ".env.development",
    )
    existing_vars: dict[str, str] = {}

    # Read any existing .env first
    ...
    # Copy from example files — these have developer-provided placeholder
    # values that are more likely to pass library validation than our generic
    # "placeholder_key_for_preview" strings.
    for example_name in example_names:
        example_path = os.path.join(frontend_dir, example_name)
        ...
        try:
            ...
            logger.info("Seeded .env from %s", example_name)
        except Exception:
            continue

    # ── Step 2: scan code for env var references ───────────────────────
    ...
    # Merge: only add vars we don't already have from .env or example files
    new_vars = env_vars - set(existing_vars.keys())

    if not existing_vars and not new_vars:
        return

    logger.info(
        "Generating .env: %d from example files, %d new placeholder vars: %s",
        len(existing_vars), len(new_vars), sorted(new_vars)[:10],
    )

    with open(env_path, "w", encoding="utf-8") as f:
        f.write("# Auto-generated by Reform for preview rendering\n")
        # Write vars seeded from example files
        for key, val in sorted(existing_vars.items()):
            f.write(f"{key}={val}\n")
        # Fill remaining with format-aware placeholders
