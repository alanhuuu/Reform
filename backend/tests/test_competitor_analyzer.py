...
class TestAnalyzeCompetitors:
    @pytest.mark.asyncio
    async def test_all_urls_succeed_calls_aggregate(self, mock_competitor_response):
        """All TinyFish calls succeed → aggregate_patterns is called with all results."""
        with (
            patch(
                "app.services.competitor_analyzer.extract_site_data",
                new_callable=AsyncMock,
            ) as mock_extract,
            patch(
                "app.services.competitor_analyzer.aggregate_patterns"
            ) as mock_aggregate,
        ):
    ...
    @pytest.mark.asyncio
    async def test_all_results_passed_to_aggregate(self, mock_competitor_response):
        ...
        with (
            ...
            await analyze_competitors(
                ["https://github.com", "https://railway.app"], ""
            )
    ...
    @pytest.mark.asyncio
    async def test_partial_failure_skips_failed_site(self, mock_competitor_response):
    ...
    @pytest.mark.asyncio
    async def test_all_failures_raises_error(self, mock_competitor_response):
    ...
    @pytest.mark.asyncio
    async def test_single_url_success(self, mock_competitor_response):
        with (
            patch(
                "app.services.competitor_analyzer.extract_site_data",
                new_callable=AsyncMock,
            ...
            result = await analyze_competitors(["https://github.com"], "test_goal")

        assert isinstance(result, CompetitorAnalysisResponse)

    @pytest.mark.asyncio
    async def test_parallel_results_not_corrupted(self, mock_competitor_response):
        """Each URL's result maps back to the correct URL (no task-level corruption)."""
        async def slow_extract(url):
            import asyncio as _asyncio
            await _asyncio.sleep(0.001)  # tiny sleep to encourage interleaving
        ...
        with (
            ...
            mock_aggregate.return_value = mock_competitor_response
            await analyze_competitors(urls, "")
    ...
    @pytest.mark.asyncio
    async def test_returns_competitor_analysis_response(self, mock_competitor_response):
        with (
            patch(
                "app.services.competitor_analyzer.extract_site_data",
                new_callable=AsyncMock,
            ...
            result = await analyze_competitors(["https://github.com"], "")

        assert isinstance(result, CompetitorAnalysisResponse)

    @pytest.mark.asyncio
    async def test_style_goal_forwarded_to_aggregate(self, mock_competitor_response):
        with (
            patch(
                "app.services.competitor_analyzer.extract_site_data",
                new_callable=AsyncMock,
            ...
            mock_aggregate.return_value = mock_competitor_response
            await analyze_competitors(["https://github.com"], "custom_goal")
